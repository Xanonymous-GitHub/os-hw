...
pthread_mutex_t _mutex, atb, bta, mutex1;
time_t start;
...
void *ATB(void *id) {
    ...
    pthread_mutex_lock(&_mutex);      // lock mutex
    ::atbcnt++;
    ...
    pthread_mutex_unlock(&_mutex);    // unlock mutex
    display(*(int *)id, "Crossing"); // stage 2

    pthread_mutex_lock(&mutex1);
    ::onatb++;
    while (::onatb > 1)
        ; // a villager may have to travel behind another villager
    ...
    pthread_mutex_lock(&_mutex); // locking mutex to decrement
    ::atbcnt--;
    ::onatb--;
